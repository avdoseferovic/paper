...
// InitType is used with NewCustom() to customize an PDF instance.
...
type PDF struct {
	...
	isRTL            bool                       // is right to left mode enabled
	...
	coreFonts        coreFontSet                // set of core font names
	...
	acroForm         *preparedAcroForm          // AcroForm objects prepared for output
	viewerPrefs      *ViewerPreferences         // catalog viewer preference hints
	pageLabels       []PageLabelRange           // catalog /PageLabels ranges
	attachments      []FileAttachment           // embedded files to emit
	attachmentRefs   []attachmentFileSpecRef    // filespec objects written for attachments
	namedDests       []NamedDestination         // catalog /Names /Dests entries
	pageAnnotations  []PageAnnotation           // configured page annotations
	...
	xmpObj           int                        // object number of the XMP metadata stream
	language         string                     // document language (/Lang)
...
type fontDefType struct {
	...
	CwExtra        map[int]int   // Sparse widths for runes outside the BMP
	Enc            string        // "cp1252", ...
...
func generateFontID(fdt fontDefType) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("marshal font definition: %w", err)
	}
