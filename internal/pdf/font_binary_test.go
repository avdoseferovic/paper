...
func TestKeySortHelpers(t *testing.T) {
	...
	if got, want := sortedKeys(map[string][]byte{"b": nil, "a": nil, "c": nil}), []string{"a", "b", "c"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("expected %v, got %v", want, got)
	}
	if got, want := sortedKeys(map[int]int{9: 0, 1: 0, 4: 0}), []int{1, 4, 9}; !reflect.DeepEqual(got, want) {
		t.Fatalf("expected %v, got %v", want, got)
	}
	if got, want := sortedKeys(map[int][]int{7: nil, 3: nil, 5: nil}), []int{3, 5, 7}; !reflect.DeepEqual(got, want) {
		t.Fatalf("expected %v, got %v", want, got)
	}
}
