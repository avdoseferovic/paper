// Package compression demonstrates turning content-stream compression on and off.
package compression
...
// GetPaper builds the compression demo document. It returns an error when the
// image cannot be read so callers decide how to fail; the builder itself must
// never terminate the process, because it also runs inside the wasm playground
// where os.Exit would kill every registered export.
func GetPaper(imagePath string) (core.Paper, error) {
