...
func TestImageRow_DefaultResolverRefusesLocalPath_FallsBackToAlt(t *testing.T) {
	...
	rows, err := Translate(t.Context(), doc) // no resolver configured → default safe resolver
	require.NoError(t, err)
...
func TestImageRow_WithImageBaseDir_LoadsLocalPNG(t *testing.T) {
	...
	rows, err := Translate(t.Context(), doc, WithImageBaseDir(dir))
	require.NoError(t, err)
...
func TestImageRow_CustomResolverIsInvoked(t *testing.T) {
	...
	_, err = Translate(t.Context(), doc, WithImageResolver(resolver))
	require.NoError(t, err)
...
func TestImageRow_SrcsetSelectsHighestDensityCandidate(t *testing.T) {
	...
	rows, err := Translate(t.Context(), doc, WithImageResolver(resolver))
	require.NoError(t, err)
	require.Len(t, rows, 1)
...
<img src="fallback.png" width="8mm" height="6mm" alt="hero">
	...
	rows, err := Translate(t.Context(), doc, WithImageResolver(resolver), WithContentWidth(80))
	require.NoError(t, err)
	require.Len(t, rows, 1)
