...
func TestFlexCenterOddSlack(t *testing.T) {
	...
	doc := parseDoc(t, `<html><body><div style="display:flex;justify-content:center"><div style="flex:0 0 25%">a</div><div style="flex:0 0 25%">b</div></div></body></html>`)
	rows, err := translate.Translate(t.Context(), doc, translate.WithGridSize(11))
	require.NoError(t, err)
	require.Len(t, rows, 1)
	cols := rows[0].GetColumns()
	require.Len(t, cols, 4)
...
func TestFlexFixedBasisOverflowScalingPreservesGrid(t *testing.T) {
	...
	rows, err := translate.Translate(t.Context(), doc, translate.WithGridSize(72))
	require.NoError(t, err)
	require.Len(t, rows, 1)
...
func TestFlexGap(t *testing.T) {
	...
	t.Run("gap shorthand is equivalent to column-gap on flex-row", func(t *testing.T) {
		...
		rowsA, _ := translate.Translate(t.Context(), docA)
		...
		rowsB, _ := translate.Translate(t.Context(), docB)
...
func TestFlexWrap_AutoSizedItemsUseEstimatedContentWidth(t *testing.T) {
	...
	rows, err := translate.Translate(t.Context(), doc, translate.WithGridSize(72), translate.WithContentWidth(180))
	require.NoError(t, err)
	require.Len(t, rows, 1)
